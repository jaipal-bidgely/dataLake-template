...
import com.bidgely.lakehouse.DeltaLake.{compactDeltaTable, createSparkSession, loadData, timeTravelQuery, vacuumDeltaTable, writeDeltaLakeTable, zOrderClustering}
...
//      s"$readPath/pilot_id=10015/event_month=2024-03-01/"
    ...
    paths.foreach { path =>
      ...
      // Write to Delta Lake
      startTime = System.nanoTime()
      writeDeltaLakeTable(spark, df, writePath_history, SaveMode.Overwrite, "pilot_id")
      endTime = System.nanoTime()
...
//    )
    ...
    paths_dedup.foreach { path_dedup =>
      ...
      // Write to Delta Lake
      startTime = System.nanoTime()
      writeDeltaLakeTable(spark, df, writePath_dedup, SaveMode.Overwrite, "pilot_id")
      endTime = System.nanoTime()
      println(s"\nDelta: Time taken for write (a month) operation: ${(endTime - startTime) / 1e9} seconds")
