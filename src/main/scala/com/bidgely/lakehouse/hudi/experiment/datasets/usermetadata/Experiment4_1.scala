...
object Experiment4_1 {
  def main(args: Array[String]): Unit = {
    ...
    val writePath_history = "s3://bidgely-lakehouse-pocs/experiments/experiment4_1/hudi/history"
    val writePath_dedup = "s3://bidgely-lakehouse-pocs/experiments/experiment4_1/hudi/dedup"
    ...
    paths.foreach { path =>
      ...
      // Write to Hudi Table
      startTime = System.nanoTime()
      if(ct < (len-1)) writeHudiTable(spark, df, writePath_history, SaveMode.Append, hudiOptions, None, None)
    ...
    paths_dedup.foreach { path_dedup =>
      ...
      // Write to Hudi Table
      startTime = System.nanoTime()
      if (ct < (len-1)) writeHudiTable(spark, df, writePath_dedup, SaveMode.Append, hudiOptions_dedup_upsert, None, None)
